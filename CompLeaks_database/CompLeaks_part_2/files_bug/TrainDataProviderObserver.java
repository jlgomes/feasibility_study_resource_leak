 ...
 * detects a change.
 */
class TrainDataProviderObserver extends ContentObserver {
	private AppWidgetManager mAppWidgetManager;
	private ComponentName mComponentName;

	TrainDataProviderObserver(AppWidgetManager mgr, ComponentName cn, Handler h) {
		super(h);
		...
		if (action.equals(REFRESH_ACTION) || action.equals(UPDATE_ACTION)) {
			...
			sWorkerQueue.post(new Runnable() {
				...
				public void run() {
					...
					int[] ids = mgr.getAppWidgetIds(new ComponentName(ctx,
							TrainWidgetProvider.class));
	...
	public void onUpdate(Context context, AppWidgetManager appWidgetManager,
		...
		for (int appWidgetId : appWidgetIds) {
			...
			final Intent refreshIntent = new Intent(context,
					TrainWidgetProvider.class);
			refreshIntent.setAction(TrainWidgetProvider.REFRESH_ACTION);
