...
public class TrainAppWidgetProvider extends AppWidgetProvider {
	...
	private ArrayList<TrainStop> listOfTrainStops = new ArrayList<TrainStop>();
	...
	public void update(Context context, RemoteViews updateViews) {
		...
		try {
			...
			if (size > 1) {
				...
				String time = mSTOPCursor.getString(mSTOPCursor
						.getColumnIndex(ConnectionDbAdapter.KEY_STOP_TIME));
				String late = mSTOPCursor.getString(mSTOPCursor
						.getColumnIndex(ConnectionDbAdapter.KEY_STOP_LATE));
				String station = mSTOPCursor.getString(mSTOPCursor
						.getColumnIndex(ConnectionDbAdapter.KEY_STOP_NAME));
				updateViews.setTextViewText(R.id.text2, station);
	...
	public void setListOfTrainStops(ArrayList<TrainStop> listOfTrainStops) {
		this.listOfTrainStops = listOfTrainStops;
	}

	public ArrayList<TrainStop> getListOfTrainStops() {
